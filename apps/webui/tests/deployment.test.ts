...
import { deploymentPresetValue, toCron } from "../src/lib/deployment";
...
describe("deployment presets", () => {
	...
	test("daily and weekdays preserve their advertised repeat rule", () => {
		expect(deploymentPresetValue("daily", monday).repeat).toBe("每天");
		expect(deploymentPresetValue("weekdays", monday).repeat).toBe("工作日");
	});

	test("weekly selects the next Monday and produces a Monday cron", () => {
		const preset = deploymentPresetValue("weekly", monday);
