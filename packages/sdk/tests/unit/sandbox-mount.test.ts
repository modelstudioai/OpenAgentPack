...
describe("resolveSandboxMountPath", () => {
	test("bailian/claude prepend /mnt/session and preserve subdirs", () => {
		expect(resolveSandboxMountPath("bailian", "/uploads/report.pdf")).toBe("/mnt/session/uploads/report.pdf");
		expect(resolveSandboxMountPath("claude", "/uploads/a/b.txt")).toBe("/mnt/session/uploads/a/b.txt");
	});

	test("collapses duplicate slashes at the join", () => {
		expect(resolveSandboxMountPath("bailian", "uploads/x.txt")).toBe("/mnt/session/uploads/x.txt");
		expect(resolveSandboxMountPath("claude", "//uploads//x.txt")).toBe("/mnt/session/uploads//x.txt");
	});

	test("qoder mounts under /data/<basename>, flattening subdirs", () => {
		expect(resolveSandboxMountPath("qoder", "/uploads/report.pdf")).toBe("/data/report.pdf");
		expect(resolveSandboxMountPath("qoder", "/uploads/nested/x.txt")).toBe("/data/x.txt");
...
describe("composeFileMountHint", () => {
	...
	test("lists resolved sandbox paths per provider", () => {
		...
		expect(hint).toContain("- /mnt/session/uploads/a.txt");
	...
	test("multiple files each get a line", () => {
		...
		expect(hint).toContain("- /data/a.txt");
		expect(hint).toContain("- /data/b.txt");
	});
});
...
describe("prependFileHint", () => {
	...
	test("hint comes before the original prompt", () => {
		...
		expect(out.indexOf("/mnt/session/uploads/a.txt")).toBeLessThan(out.indexOf("summarize it"));
	});
});
...
describe("rewriteFileMentions", () => {
	test("replaces sentinel with sandbox path", () => {
		...
		expect(rewriteFileMentions(prompt, "bailian")).toBe("see /mnt/session/uploads/a.png here");
	});
...
describe("preparePromptForProvider", () => {
	test("rewrites sentinel then prepends hint", () => {
		...
		expect(out).toContain("/mnt/session/uploads/a.txt");
